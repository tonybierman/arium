//! All `arium-dioxus` server fns.
//!
//! Consumers do `use arium_dioxus::server::*;` once at the top of their app so
//! the Dioxus `#[post(...)]` / `#[get(...)]` macro registrations link, then the
//! existing client-side call sites (`login_with_password(...).await`) just
//! work.
//!
//! The axum-level OAuth handlers (which aren't Dioxus server fns) live in the
//! engine's `arium::oauth` module (when the `_oauth-core` feature is on) and are
//! mounted by `arium::install`.
...
#[cfg(feature = "server")]
pub(crate) type DbExtension = axum::Extension<arium::pool::Pool>;

#[cfg(all(feature = "server", feature = "mail"))]
pub(crate) type MailExtension = axum::Extension<arium::mail::Mailer>;

#[cfg(feature = "server")]
pub(crate) type ProvidersExtension = axum::Extension<std::sync::Arc<Vec<arium_wire::ProviderInfo>>>;

// `AuditCtx` (the IP/User-Agent audit extractor) and `SessionStore` (the
// per-request session handle) are framework-agnostic axum primitives that now
